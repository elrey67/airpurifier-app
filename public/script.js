...
function initializeEventListeners() {
    // Control event listeners - with null checks
    ...
    if (thresholdSlider) thresholdSlider.addEventListener('input', updateThreshold);
...
function updateData() {
    fetch('/data')
        ...
        .then(data => {
            ...
            const status = systemMode === 'online' ? 'connected' : 'disconnected';
            const message = systemMode === 'online' 
                ? 'Connected to cloud database' 
                : 'Running on local sensor data';
...
function attemptReconnect() {
    if (reconnectAttempts < maxReconnectAttempts) {
        ...
        updateConnectionStatus('failed', 'Failed to connect to device');
    }
}
...
function updateAllDisplays() {
    ...
    if (efficiencyValue) {
        if (efficiency > 70) {
            efficiencyValue.style.color = 'var(--success)';
        } else if (efficiency > 40) {
            efficiencyValue.style.color = 'var(--warning)';
        } else {
            efficiencyValue.style.color = 'var(--danger)';
        }
    }
    
    // Update gauges
    updateGauge('input', inputAirQuality);
    updateGauge('output', outputAirQuality);
...
function updateGauge(type, value) {
    ...
    if (needle && valueElement) {
        ...
        updateValueColorElement(valueElement, value);
    }
}
...
function updateValueColor(elementId, value) {
    const element = document.getElementById(elementId);
    if (element) {
        updateValueColorElement(element, value);
    }
}
...
function updateComparisonChart() {
    ...
    if (inputBar && outputBar && improvementElement) {
        ...
        updateValueColorElement(inputBar, inputAirQuality);
        updateValueColorElement(outputBar, outputAirQuality);
    }
}
...
function updateLastUpdated() {
    ...
    if (lastUpdatedElement) {
        lastUpdatedElement.textContent = 
            `Last updated: ${lastUpdateTime.toLocaleTimeString()} (${displayText}) | Mode: ${systemMode.toUpperCase()}`;
    }
}
